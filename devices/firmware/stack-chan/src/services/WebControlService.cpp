...
.error{color:#ff8d8d}.footer{color:var(--muted);margin-top:14px;font-size:11px}@media(max-width:720px){main{padding:14px}.grid{grid-template-columns:1fr}.top{align-items:start;flex-direction:column}.status{text-align:left}}
...
<section class="card"><h2>Expression</h2><div class="buttons" id="expressions"></div><h2 style="margin-top:18px">Body LEDs</h2><div class="rgb"><input id="color" type="color" value="#000020"><button id="setColor">Set</button></div></section></div>
<div class="footer">Local control only · movement is clamped in firmware · update rate 5 Hz</div></main><script>
const $=id=>document.getElementById(id), expressions=['neutral','happy','listening','thinking','speaking','surprised','sleepy'];let state={},lastMove=0,torque=true;
...
$('setColor').onclick=()=>{const c=$('color').value;post('/api/rgb',{r:parseInt(c.slice(1,3),16),g:parseInt(c.slice(3,5),16),b:parseInt(c.slice(5,7),16)})};poll();
</script></body></html>
)HTML";
