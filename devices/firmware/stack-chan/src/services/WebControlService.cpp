...
<section class="card audio-card"><h2>Walkie-talkie</h2><div class="audio-grid"><div class="audio-panel"><h3>Stack-Chan → this browser</h3><p>Enable listening, then press and hold Stack-Chan's top touch control to talk. Release it to play the message here.</p><div class="buttons"><button id="pttEnable">Enable walkie-talkie</button><button id="deviceRecord">Record device · 5 s</button><button id="microphoneToggle">Mute device mic</button></div><audio class="audio-player" id="deviceAudio" controls hidden></audio><div class="audio-status" id="deviceAudioStatus">Enable walkie-talkie to receive</div></div><div class="audio-panel"><h3>This browser → Stack-Chan</h3><p>Record up to ten seconds here, or choose an audio recording. Pressing Stack-Chan's top control interrupts this playback.</p><div class="buttons"><button id="browserRecord">Start recording</button><label class="file-button">Choose / record audio<input id="audioFile" type="file" accept="audio/*" capture></label><button id="speakerToggle">Mute device speaker</button></div><div class="audio-status" id="browserAudioStatus">Ready</div></div></div></section></div>
...
const $=id=>document.getElementById(id), expressions=['neutral','happy','listening','thinking','speaking','surprised','sleepy'],audioBase=`${location.protocol}//${location.hostname}:82`;let state={},lastMove=0,torque=true,browserRecording=null,deviceAudioUrl='',pttSequence=null,pttPending=0,pttEnabled=false,pttBusy=false,pttContext=null;
...
function render(s){state=s;$('connection').textContent=`${s.mode} · ${s.ip} · ${s.moving?'moving':'idle'}`;$('battery').textContent=s.battery_percent<0?'--':s.battery_percent+'%';$('yaw').textContent=s.yaw.toFixed(1)+'°';$('pitch').textContent=s.pitch.toFixed(1)+'°';for(let i=0;i<3;i++)$('t'+i).dataset.v=s.touch[i];document.querySelectorAll('[data-expression]').forEach(b=>b.classList.toggle('active',b.dataset.expression===s.expression));$('dot').style.left=((s.yaw+120)/240*100)+'%';$('dot').style.top=((85-s.pitch)/80*100)+'%';$('microphoneToggle').textContent=s.microphone_enabled?'Mute device mic':'Unmute device mic';$('speakerToggle').textContent=s.speaker_enabled?'Mute device speaker':'Unmute device speaker';$('microphoneToggle').classList.toggle('active',!s.microphone_enabled);$('speakerToggle').classList.toggle('active',!s.speaker_enabled);$('deviceRecord').disabled=!s.audio_available||!s.microphone_enabled||s.audio_recording;$('browserRecord').disabled=!s.audio_available||!s.speaker_enabled||(!browserRecording&&(s.audio_playing||s.audio_recording));if(s.push_to_talk)$('deviceAudioStatus').textContent='Top control pressed · recording…';if(pttSequence===null)pttSequence=s.ptt_sequence;else if(s.ptt_sequence!==pttSequence){pttSequence=s.ptt_sequence;pttPending=pttSequence;if(pttEnabled)playPushToTalk(pttSequence);else $('deviceAudioStatus').textContent='Message ready · enable walkie-talkie to hear it'}setVideo(s.camera)}
...
async function playPushToTalk(sequence){if(!pttEnabled||pttBusy)return;pttBusy=true;const status=$('deviceAudioStatus');status.className='audio-status';status.textContent='Receiving message…';try{const response=await fetch(audioBase+'/ptt/latest',{cache:'no-store'});if(!response.ok)throw Error(await response.text());const blob=await response.blob();if(!blob.size)throw Error('No push-to-talk message is ready');if(deviceAudioUrl)URL.revokeObjectURL(deviceAudioUrl);deviceAudioUrl=URL.createObjectURL(blob);$('deviceAudio').src=deviceAudioUrl;$('deviceAudio').hidden=false;const decoded=await pttContext.decodeAudioData(await blob.arrayBuffer()),source=pttContext.createBufferSource();source.buffer=decoded;source.connect(pttContext.destination);source.start();pttPending=0;status.textContent=`Message ${sequence} · playing` }catch(error){showDeviceError(error)}finally{pttBusy=false}}
$('pttEnable').onclick=async()=>{try{if(!pttContext)pttContext=new (window.AudioContext||window.webkitAudioContext)();await pttContext.resume();pttEnabled=true;$('pttEnable').textContent='Walkie-talkie enabled';$('pttEnable').classList.add('active');$('deviceAudioStatus').className='audio-status';$('deviceAudioStatus').textContent='Listening for Stack-Chan';if(pttPending)playPushToTalk(pttPending)}catch(error){showDeviceError(error)}};
...
void WebControlService::sendState() {
  ...
  json += ",\"push_to_talk\":" +
          String(_audio->pushToTalkActive() ? "true" : "false");
  json += ",\"ptt_sequence\":" + String(_audio->pushToTalkSequence());
